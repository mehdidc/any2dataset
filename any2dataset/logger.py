...
class SpeedLogger(Logger):
    ...
    def do_log(
        ...
        print(
            " - ".join(
                [
                    ...
                    f"files per sec: {file_per_sec:.0f}",
                    f"count: {count}",
                ]
            )
        )
        ...
        if self.enable_wandb:
            wandb.log(
                {
                    f"{self.prefix}/file_per_sec": file_per_sec,
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                stats_files = [f for f in stats_files if int(f.split("/")[-1].split("_")[0].replace(".json", "")  ) >= self.start_shard_id]
